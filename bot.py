...
API_TOKEN = '185003641:AAH0W8Vm_5ghV6Y5s8yvRxwCUdX5TxNXB2w'
...
def on_user_joins(m):
	...
	if m.content_type == 'new_chat_participant':
		if m.new_chat_participant.id == bot.get_me().id:
			...
			groupid = m.chat.id
			bot.send_message(92983555, "# DEBUG # " + "Bot got invited to the group " + str(groupname) + "(" + str(groupid) + ")", parse_mode="HTML")
...
def yes(m):
	...
	bot.send_message(92983555, str(user)  + " had Problems in setting up." , parse_mode="Markdown")
...
def no(m):
	cid = m.chat.id
	user = m.from_user.first_name
	bot.send_message(92983555, str(user) + " had no Problems in setting the Bot up." , parse_mode="Markdown")
	bot.send_message(cid, "Thank you very much for making our Bot better!")
